...
│   └── design-params.ts           # MountingBracketParams | EnclosureParams | OrganizerParams
...
- **Claude tools:** `extract_design_parameters`, `analyze_photo`, `infer_dimensions`, `request_clarification`
- **System prompt** in `prompts/design_wizard.py`: conversation flow (WHAT → WHY → WHERE → SPECIFICS) + reference object calibration
